...
func (s *HelmSuite) TestLoadChart_HasRequiredMetadata() {
	if s.sampleVersion == "" {
	...
	chart, err := s.client.LoadChart(ctx, s.sampleRepo, s.sampleChart, s.sampleVersion)
	if err != nil && strings.Contains(err.Error(), "403") {
		s.T().Skipf("Skipping due to registry auth: %v", err)
	}
	s.Require().NoError(err)

	// Contract: Chart must have valid metadata
	s.Equal(s.sampleChart, chart.Name, "Chart name should match request")
	s.Equal(s.sampleVersion, chart.Version, "Chart version should match request")
	s.NotEmpty(chart.Raw, "Chart should have raw files")
}

func (s *HelmSuite) TestLoadChart_HasChartYaml() {
	if s.sampleVersion == "" {
		s.T().Skip("No sample version available")
	}
	...
	// Contract: Every Helm chart must have Chart.yaml
	hasChartYaml := false
	for _, f := range chart.Raw {
		if f.Name == "Chart.yaml" {
			hasChartYaml = true
			s.NotEmpty(f.Data, "Chart.yaml should not be empty")
			break
		}
	}
	s.True(hasChartYaml, "Chart must contain Chart.yaml")
}
...
func (s *HelmSuite) TestVersionNotFound_ReturnsError() {
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()

	_, err := s.client.LoadChart(ctx, s.sampleRepo, s.sampleChart, "0.0.0-nonexistent")
	if err != nil && strings.Contains(err.Error(), "403") {
		s.T().Skipf("Skipping due to registry auth: %v", err)
	}

	s.Require().Error(err, "Non-existent version should return error")
}
...
func (s *HelmSuite) TestRefreshIndex_InvalidatesCache() {
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()

	// Populate cache
	_, err := s.client.ListCharts(ctx, s.sampleRepo)
	s.Require().NoError(err)

	// Refresh should succeed
	err = s.client.RefreshIndex(ctx, s.sampleRepo)
	s.Require().NoError(err)

	// Subsequent call should still work (cache was invalidated and repopulated)
	charts, err := s.client.ListCharts(ctx, s.sampleRepo)
	s.Require().NoError(err)
	s.NotEmpty(charts, "Should still return charts after refresh")
}
